...
package net.mcreator.luckyblock.world.dimension;
...
import net.mcreator.luckyblock.item.ParadiceItem;
import net.mcreator.luckyblock.block.OBSIDIANBlock;
...
public class ParadiceDimension extends LuckyBlockModElements.ModElement {
	@ObjectHolder("lucky_block:paradice_portal")
	public static final CustomPortalBlock portal = null;
	public ParadiceDimension(LuckyBlockModElements instance) {
		super(instance, 32);
		...
		DeferredWorkQueue.runLater(() -> {
			try {
				ObfuscationReflectionHelper.setPrivateValue(WorldCarver.class, WorldCarver.CAVE, new ImmutableSet.Builder<Block>()
						...
						.add(Blocks.DIAMOND_BLOCK.getDefaultState().getBlock()).build(), "field_222718_j");
	...
	public void clientLoad(FMLClientSetupEvent event) {
		DimensionRenderInfo customEffect = new DimensionRenderInfo(Float.NaN, true, DimensionRenderInfo.FogType.NONE, false, false) {
			...
			public Vector3d func_230494_a_(Vector3d color, float sunHeight) {
				return new Vector3d(0.6, 0.6, 0.6);
			}
		...
		DeferredWorkQueue.runLater(() -> {
			try {
				...
				effectsRegistry.put(new ResourceLocation("lucky_block:paradice"), customEffect);
			} catch (Exception e) {
				e.printStackTrace();
			}
		});
	...
	public static final TicketType<BlockPos> CUSTOM_PORTAL = TicketType.create("paradice_portal", Vector3i::compareTo, 300);
	public static class POIRegisterHandler {
		...
		public void registerPointOfInterest(RegistryEvent.Register<PointOfInterestType> event) {
			poi = new PointOfInterestType("paradice_portal", Sets.newHashSet(ImmutableSet.copyOf(portal.getStateContainer().getValidStates())), 0, 1)
					.setRegistryName("paradice_portal");
	...
	public void initElements() {
		...
		elements.items.add(() -> new ParadiceItem().setRegistryName("paradice"));
	}
			...
			if (!entity.isPassenger() && !entity.isBeingRidden() && entity.isNonBoss() && !entity.world.isRemote && true) {
				if (entity.func_242280_ah()) {
					entity.func_242279_ag();
						...
						new ResourceLocation("lucky_block:paradice"))) {
					entity.func_242279_ag();
					teleportToDimension(entity, pos, RegistryKey.getOrCreateKey(Registry.WORLD_KEY, new ResourceLocation("lucky_block:paradice")));
				} else {
					entity.func_242279_ag();
	...
	public static class CustomPortalSize {
		private static final AbstractBlock.IPositionPredicate POSITION_PREDICATE = (state, blockReader, pos) -> {
			return state.getBlock() == OBSIDIANBlock.block.getDefaultState().getBlock();
		};
					...
					for (int k2 = 0; k2 < 2; ++k2) {
						for (int i3 = -1; i3 < 3; ++i3) {
							BlockState blockstate1 = i3 < 0
									? OBSIDIANBlock.block.getDefaultState().getBlock().getDefaultState()
									: Blocks.AIR.getDefaultState();
			...
			for (int k1 = -1; k1 < 3; ++k1) {
				for (int i2 = -1; i2 < 4; ++i2) {
					if (k1 == -1 || k1 == 2 || i2 == -1 || i2 == 3) {
						...
						this.world.setBlockState(blockpos$mutable, OBSIDIANBlock.block.getDefaultState().getBlock().getDefaultState(), 3);
					}
				}
			}
