...
package net.mcreator.thegoatedmod.world.dimension;
...
import net.mcreator.thegoatedmod.item.SquidboysforlifeItem;
import net.mcreator.thegoatedmod.block.SquidFrameBlock;
import net.mcreator.thegoatedmod.block.PlatnumBlockBlock;
...
public class SquidboysforlifeDimension extends TheGoatedModModElements.ModElement {
	@ObjectHolder("the_goated_mod:squidboysforlife_portal")
	public static final CustomPortalBlock portal = null;
	public SquidboysforlifeDimension(TheGoatedModModElements instance) {
		super(instance, 31);
		...
		DeferredWorkQueue.runLater(() -> {
			try {
				ObfuscationReflectionHelper.setPrivateValue(WorldCarver.class, WorldCarver.CAVE, new ImmutableSet.Builder<Block>()
						...
						.add(PlatnumBlockBlock.block.getDefaultState().getBlock()).build(), "field_222718_j");
	...
	public void clientLoad(FMLClientSetupEvent event) {
		...
		DeferredWorkQueue.runLater(() -> {
			try {
				...
				effectsRegistry.put(new ResourceLocation("the_goated_mod:squidboysforlife"), customEffect);
			} catch (Exception e) {
				e.printStackTrace();
			}
		});
	...
	public static final TicketType<BlockPos> CUSTOM_PORTAL = TicketType.create("squidboysforlife_portal", Vector3i::compareTo, 300);
	public static class POIRegisterHandler {
		...
		public void registerPointOfInterest(RegistryEvent.Register<PointOfInterestType> event) {
			poi = new PointOfInterestType("squidboysforlife_portal",
					Sets.newHashSet(ImmutableSet.copyOf(portal.getStateContainer().getValidStates())), 0, 1)
							.setRegistryName("squidboysforlife_portal");
	...
	public void initElements() {
		...
		elements.items.add(() -> new SquidboysforlifeItem().setRegistryName("squidboysforlife"));
	}
	public static class CustomPortalBlock extends NetherPortalBlock {
		...
		public void animateTick(BlockState state, World world, BlockPos pos, Random random) {
			for (int i = 0; i < 4; i++) {
				...
				world.addParticle(ParticleTypes.FISHING, px, py, pz, vx, vy, vz);
			}
			...
			if (!entity.isPassenger() && !entity.isBeingRidden() && entity.isNonBoss() && !entity.world.isRemote && true) {
				if (entity.func_242280_ah()) {
					entity.func_242279_ag();
						...
						new ResourceLocation("the_goated_mod:squidboysforlife"))) {
					entity.func_242279_ag();
					teleportToDimension(entity, pos,
							RegistryKey.getOrCreateKey(Registry.WORLD_KEY, new ResourceLocation("the_goated_mod:squidboysforlife")));
				} else {
					entity.func_242279_ag();
	...
	public static class CustomPortalSize {
		private static final AbstractBlock.IPositionPredicate POSITION_PREDICATE = (state, blockReader, pos) -> {
			return state.getBlock() == SquidFrameBlock.block.getDefaultState().getBlock();
		};
					...
					for (int k2 = 0; k2 < 2; ++k2) {
						for (int i3 = -1; i3 < 3; ++i3) {
							BlockState blockstate1 = i3 < 0
									? SquidFrameBlock.block.getDefaultState().getBlock().getDefaultState()
									: Blocks.AIR.getDefaultState();
			...
			for (int k1 = -1; k1 < 3; ++k1) {
				for (int i2 = -1; i2 < 4; ++i2) {
					if (k1 == -1 || k1 == 2 || i2 == -1 || i2 == 3) {
						...
						this.world.setBlockState(blockpos$mutable, SquidFrameBlock.block.getDefaultState().getBlock().getDefaultState(), 3);
					}
				}
			}
